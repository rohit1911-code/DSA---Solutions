...
int main() {
    ...
    int hs = 0;
    for (int i = 0; i < n; i++) {
        scanf("%d %dn", &zas[i].when, &zas[i].how_long);
    }
    ...
    while (zap < n || hs) {
        while (zap < n && zas[zap].when <= next_ready) {
            ...
            zash[hs++] = zas[zap];
            fprintf(stderr, "push: %d %dn", zas[zap].when, zas[zap].how_long);
        ...
        if (!hs) {
          ...
          fprintf(stderr, "pop: %d %dn", cookme.when, cookme.how_long);
            next_ready = next_ready + cookme.how_long;
            tt += next_ready - cookme.when;
        }
    }
    fprintf(stderr, "%ld %ldn", tt, tt1);
    if (tt1 != tt) exit(-1);
    printf("%ldn", tt / n);
    return 0;
}